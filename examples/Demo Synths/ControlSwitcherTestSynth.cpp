	...
	ControlSwitcherTestSynth()
	{
		...
		auto makeBass = [&](Generator bassFreq)
		{
			return RectWave()
				...
				.freq(bassFreq + bassFreq * SineWave().
					freq(bassFreq * 1.99) * (fmAmount * 0.7 + addParameter("addt1FM")).smoothed()
				);
		};

		Generator bass = (makeBass(freq) >> MonoToStereoPanner().pan(-1 * spread)) +
			(makeBass(freq * 1.02) >> MonoToStereoPanner().pan(1 * spread))
...
TONALITY_REGISTER_SYNTH(ControlSwitcherTestSynth);