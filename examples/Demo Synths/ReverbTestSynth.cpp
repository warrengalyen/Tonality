...
class ReverbTestSynth : public Synth
{
	ReverbTestSynth()
	{
		...
		Generator tone = RectWave().pwm(0.5f).freq(Tonality::mtof(60)) * ADSR(0.001f, 0.08f, 0, 0.01f).doesSustain(false).exponential(true).trigger(offbeat);
...
TONALITY_REGISTER_SYNTH(ReverbTestSynth);