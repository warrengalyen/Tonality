...
public:
	ControlSnapToScaleTestSynth()
	{
		...
		Generator outputGen = ((
			...
			SineWave().freq(
				ControlMidiToFreq().input(scaleSnapper1 + 12)) * 0.1) * 0.3 * env) >>
			
		StereoDelay(0.3, 0.33).feedback(0.3).dryLevel(1.0 - delayMix).wetLevel(delayMix) >> verb;

		setOutputGen(outputGen);
	}
};

TONALITY_REGISTER_SYNTH(ControlSnapToScaleTestSynth);