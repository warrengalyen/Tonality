		...
		void ADSR_::switchState(ADSRState newState) {
			...
			case ATTACK: {
				...
				if (segLength == 0) {
					...
					targetValue = 1.0f;
					increment = (TonalityFloat)(1.0f - lastValue) / segLength;
				}
			...
			case DECAY: {
				...
				if (segLength == 0) {
					...
					increment = (TonalityFloat)(sustainLevelVal - lastValue) / segLength;
				}
			...
			case RELEASE: {
				...
				if (segLength == 0) {
					...
					increment = (TonalityFloat)(-lastValue) / segLength;
				}
			}
						  break;
