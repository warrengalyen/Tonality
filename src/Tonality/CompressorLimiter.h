...
namespace Tonality {
	...
	namespace Tonality_ {
		...
		protected:
			...
			TonalityFrames ampInputFrames_;

			TonalityFloat ampEnvValue_;
			TonalityFloat gainEnvValue_;
		...
		public:
			...
			void tick(TonalityFrames &frames, const SynthesisContext_ & context);
			void tickThrough(TonalityFrames & inFrames, TonalityFrames & outFrames, const SynthesisContext_ & context);
		...
		inline void Compressor_::tick(TonalityFrames &frames, const SynthesisContext_ &context) {
		...
		inline void Compressor_::tickThrough(TonalityFrames & inFrames, TonalityFrames & outFrames, const SynthesisContext_ & context) {
		...
		inline void Compressor_::computeSynthesisBlock(const SynthesisContext_ &context) {
			...
			TonalityFloat * ampData = &ampInputFrames_[0];
			...
			TonalityFloat ampInputValue, gainValue, gainTarget;
			...
			TonalityFloat makeupGain = max(0.f, makeupGainGen_.tick(context).value);
	...
	class Compressor : public TemplatedEffect<Compressor, Tonality_::Compressor_> {
	...
	public:
		...
		TONALITY_MAKE_CTRL_GEN_SETTERS(Compressor, attack, setAttack);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Compressor, release, setRelease);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Compressor, threshold, setThreshold); // LINEAR - use dBToLin to convert from dB
		TONALITY_MAKE_CTRL_GEN_SETTERS(Compressor, ratio, setRatio);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Compressor, lookahead, setLookahead);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Compressor, makeupGain, setMakeupGain);
	...
	class Limiter : public TemplatedEffect<Limiter, Tonality_::Compressor_> {
	...
	public:
		...
		TONALITY_MAKE_CTRL_GEN_SETTERS(Limiter, release, setRelease);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Limiter, threshold, setThreshold);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Limiter, lookahead, setLookahead);
		TONALITY_MAKE_CTRL_GEN_SETTERS(Limiter, makeupGain, setMakeupGain);
