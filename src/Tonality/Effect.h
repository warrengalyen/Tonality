...
namespace Tonality {
	...
	namespace Tonality_ {
		...
		public:
			...
			*/
			virtual void tickThrough(TonalityFrames & inFrames, TonalityFrames & outFrames, const SynthesisContext_ & context);
		...
		inline void Effect_::tick(TonalityFrames &frames, const SynthesisContext_ &context) {
			...
			if (!isfinite(frames(0, 0))) {
				Tonality::error("Effect_::tick NaN or inf detected.");
			}
#endif

		}

		inline void Effect_::tickThrough(TonalityFrames & inFrames, TonalityFrames & outFrames, const SynthesisContext_ & context) {
	...
	public:
		...
		void tickThrough(TonalityFrames & inFrames, const Tonality_::SynthesisContext_ & context) { // ticks in-place
			this->gen()->tickThrough(inFrames, inFrames, context);
		}

		void tickThrough(TonalityFrames & inFrames, TonalityFrames & outFrames, const Tonality_::SynthesisContext_ & context) {
		...
		TONALITY_MAKE_CTRL_GEN_SETTERS(EffectType, bypass, setBypassCtrlGen);
	...
	namespace Tonality_
	{
		...
		// subclasses don't need to tick input - dryFrames_ contains "dry" input by the time
		// computeSynthesisBlock() is called
		inline void WetDryEffect_::tick(TonalityFrames &frames, const SynthesisContext_ &context) {
		...
		inline void WetDryEffect_::tickThrough(TonalityFrames & inFrames, TonalityFrames & outFrames, const SynthesisContext_ & context) {
	...
	public:
		...
		// Defaults to 1.0
		TONALITY_MAKE_GEN_SETTERS(EffectType, wetLevel, setWetLevelGen);

		// Defaults to 0.0 (full wet)
		TONALITY_MAKE_GEN_SETTERS(EffectType, dryLevel, setDryLevelGen);
