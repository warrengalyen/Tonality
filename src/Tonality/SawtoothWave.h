...
namespace Tonality {
	...
	namespace Tonality_ {
		...
		inline void AngularWave_::computeSynthesisBlock(const SynthesisContext_ &context) {
			...
			TonalityFloat const rateConstant = TONALITY_SAW_RES / Tonality::sampleRate();

			TonalityFloat slope, frac, phase;
			TonalityFloat *outptr = &outputFrames_[0];
			TonalityFloat *freqptr = &freqFrames_[0];
			TonalityFloat *slopeptr = &slopeFrames_[0];
			...
			for (unsigned int i = 0; i < outputFrames_.frames(); i++) {
				...
				// update the slope
				slope = clamp(*slopeptr++, 0.0f, 1.0f) * TONALITY_SAW_RES;

				sd.d = ps;
				ps += *freqptr++;
				offs = sd.i[1] & (TONALITY_SAW_RES - 1);
				sd.i[1] = msbi;
				...
				// on the falling edge
					...
					*outptr++ = (1.0f - ((phase - slope) / (TONALITY_SAW_RES - slope))) * 2.0f - 1.0f;
				}

			}

			sd.d = BIT32DECPT * TONALITY_SAW_RES;
			msbi = sd.i[1];
			sd.d = ps + (BIT32DECPT * TONALITY_SAW_RES - BIT32DECPT);
			sd.i[1] = msbi;
			phaseAccum_ = sd.d - BIT32DECPT * TONALITY_SAW_RES;
		...
		inline void SawtoothWaveBL_::computeSynthesisBlock(const Tonality_::SynthesisContext_ &context)
		{
	...
	*/
	class SawtoothWave : public TemplatedGenerator<Tonality_::AngularWave_>
	{

	public:

		TONALITY_MAKE_GEN_SETTERS(SawtoothWave, freq, setFrequencyGenerator);
	...
	class SawtoothWaveBL : public TemplatedGenerator<Tonality_::SawtoothWaveBL_>
	{

	public:
		TONALITY_MAKE_GEN_SETTERS(SawtoothWaveBL, freq, setFreqGen);
