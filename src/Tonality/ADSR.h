...
namespace Tonality {
	...
	namespace Tonality_ {
		...
		protected:
			...
			TonalityFloat attackTime;
			TonalityFloat decayTime;
			TonalityFloat sustainLevelVal;
			TonalityFloat releaseTime;
			bool       bIsLegato;
			bool       bDoesSustain;
			bool       bIsExponential;
			...
			TonalityFloat targetValue;
			TonalityFloat lastValue;
			TonalityFloat increment;
			TonalityFloat pole;
...
// starting point
	...
	*/
	...
	class ADSR : public TemplatedGenerator<Tonality_::ADSR_> {

	public:

		ADSR(float attack = 0.001f, float decay = 0.03f, float sustain = 1.0f, float release = 0.05f);

		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, trigger, setTrigger);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, attack, setAttack);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, decay, setDecay);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, sustain, setSustain);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, release, setRelease);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, exponential, setIsExponential);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, doesSustain, setDoesSustain);
		TONALITY_MAKE_CTRL_GEN_SETTERS(ADSR, legato, setIsLegato);
