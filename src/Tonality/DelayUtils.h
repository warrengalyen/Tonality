...
namespace Tonality
{
	// TonalityFrames subclass with ability to tick in and out. Allows random-access/multi-tap/etc.
	class DelayLine : public TonalityFrames {
	...
	public:
		...
		*/
		inline TonalityFloat tickOut(float delayTime, unsigned int channel = 0) {

			if (delayTime != lastDelayTime_) {
				float dSamp = clamp(delayTime * Tonality::sampleRate(), 0, nFrames_);
		...
		/*
		 NOTE: No bounds checking on channel index. Improper use will result in access outside array bounds.
		*/
		inline void tickIn(TonalityFloat sample, unsigned int channel = 0) {
